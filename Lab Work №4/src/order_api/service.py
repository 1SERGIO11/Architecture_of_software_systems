...
from order_api.errors import ConflictError, NotFoundError, ValidationError
...
from order_api.repositories import CustomerRepository, NotificationRepository, OrderRepository
...
class OrderApiService:
    ...
    def create_customer(self, payload: dict[str, object]) -> Customer:
        ...
        if self._customers.get(customer_id):
            raise ConflictError(f"Клиент {customer_id} уже существует.")
    ...
    def create_order(self, payload: CreateOrderPayload) -> Order:
        if self._orders.get(payload.order_id):
            raise ConflictError(f"Заказ {payload.order_id} уже существует.")

        customer = self._customers.get(payload.customer_id)
        if not customer:
            raise ValidationError(f"Нельзя создать заказ: клиент {payload.customer_id} не найден.")
    ...
    def delete_order(self, order_id: str) -> None:
        if not self._orders.delete(order_id):
            raise NotFoundError(f"Заказ {order_id} не найден.")

    def list_notifications(self, order_id: str) -> list[NotificationRecord]:
        self.get_order(order_id)
        return self._notifications.list_by_order(order_id)
...
def _render_message(order: Order) -> str:
    ...
    title = title_map[order.status]
    return f"{title}: заказ #{order.order_id}, статус {order.status.value}."
